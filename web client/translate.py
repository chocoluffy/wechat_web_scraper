...
def main():
    # Get our data as an array: [title, author, date, url, content] from read_in()
    lines = read_in()
    title = lines[0]
    author = lines[1]
    date = lines[2]
    url = lines[3]
    chinese_blob = TextBlob(lines[4])
    ...
    r = requests.get("http://localhost:5000/update", headers={'X-API-TOKEN': 'FOOBAR1'}, data={'title': title, 'author': author, 'date': date, 'url': url, 'content': str(en_content)})
    info("INFO: " + r.text)
    ...
    r_predict = requests.post("http://localhost:5000/predict", headers={'X-API-TOKEN': 'FOOBAR1'}, data={'item': '-1', 'num': 3, 'data-url': 'backup.csv'})
    info("INFO: <br>" + r_predict.text)
